...
// Compile using:
// nvcc Ch3_greyscale.cu -o greyscale -ljpeg
...
int main(int argc, char *argv[]) {
    ...
    cudaMemcpy(grey_buffer, d_grey_buffer, width*height, cudaMemcpyDeviceToHost);
    t1 = wctime() - t1;
    ...
    // Write the grey map to a file.
    fd = open("grey_output.pgm", O_CREAT | O_WRONLY, 0666);
    char buf[1024];
    rc = snprintf(buf, sizeof(buf), "P5 %d %d 255\n", width, height);
    write(fd, buf, rc);  // Write the PGM image header before data
    write(fd, grey_buffer, width * height);  // Write out all pixel data
    close(fd);
